...
class TrainCallback(BaseCallback):
    ...
    def on_ep_end(self, _locals):
        ...
        print(
            f'epoch:{ep+1}/{n_train_epochs}  train_loss:{ep_train_loss:.4f} val_loss:{ep_val_loss:.4f} best_loss:{self.best_loss:.4f} current lr: {model.optim.param_groups[0]["lr"]} time p.e.: {time()-self.t0:.3f}     ',
            end="\r",
            flush=True,
        )
